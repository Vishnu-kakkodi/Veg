...
import 'package:veegify/services/Payment/payment_service.dart';
import 'package:veegify/services/Payment/payment_service_factory.dart';
...
class _CheckoutScreenWebState extends State<CheckoutScreenWeb> {
  ...
  void _handlePaymentSuccess(Map<String, dynamic> response) {
    ...
    _createOrder(transactionId: response['razorpay_payment_id']);
  }
  ...
  Future<void> _initiatePayment() async {
    ...
    await _paymentService.initiatePayment(
      amount: cartProvider.totalPayable,
      email: user?.email ?? '',
      contact: "9666317749",
      onSuccess: (response) {
  ...
  Future<void> _createOrder({String? transactionId}) async {
    try {
      ...
      if (result['success']) {
        ...
        if (mounted) {
          setState(() => _isProcessingOrder = false);
          _showSnackBar(result['message'] ?? 'Failed to place order', Colors.red);
        }
      }
    } catch (e) {
      if (mounted) {
