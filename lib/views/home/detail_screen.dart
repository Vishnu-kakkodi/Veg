...
class _DetailScreenState extends State<DetailScreen> {
  ...
  Future<void> _fetchProductDetails() async {
    try {
      final response = await http.get(
        Uri.parse(
          'https://api.vegiffyy.com/api/getsingleproduct/${widget.productId}',
        ),
      );

      print("llllllllllllllllllllllllllllllllll${response.body}");
  ...
  Future<void> _editReviewOnServer({
    ...
    setState(() => _isReviewActionLoading = true);
    try {
      final response = await http.put(
        Uri.parse('https://api.vegiffyy.com/api/editprodutreview'),
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> reviews = List<dynamic>.from(product['reviews'] ?? []);
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Failed to update review (${response.statusCode})')),
          );
        }
      }
    } catch (e) {
      ...
      if (mounted) {
        setState(() => _isReviewActionLoading = false);
      }
    }
  }
  ...
  Future<void> _deleteReviewOnServer({required String reviewId}) async {
    ...
    try {
      final response = await http.delete(
        Uri.parse('https://api.vegiffyy.com/api/deleteproductreview'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          "productId": widget.productId,
          "userId": userId,
  ...
  Widget _buildContent(ThemeData theme) {
    ...
    // WEB LAYOUT - Two column with sticky add to cart
    if (isDesktop || isTablet) {
      return Row(
        children: [
          ...
          // RIGHT: Content + sticky bottom bar
          Expanded(
            flex: isDesktop ? 6 : 5,
            child: Column(
              children: [
