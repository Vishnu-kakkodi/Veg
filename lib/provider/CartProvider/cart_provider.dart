...
//   // NEW: Coupon info from chargeCalculations
  ...
  Future<void> loadCart(String? userId) async {
    ...
    try {
      ...
      if (!_isSameCart(_cart, newCart) || 
        ...
        notifyListeners(); // Only notify when something changed
      } else {
  ...
  Future<bool> addItemToCart({
    ...
    try {
      ...
      if (response.success) {
        ...
        _appliedCoupon = response.appliedCoupon;
        print("✅ Item added successfully");
        notifyListeners(); // Always notify after user action
        _setLoadingState(false);
        return true;
      } else {
        _error = response.message;
  ...
  Future<bool> incrementQuantity(String cartProductId, String? userIdd) async {
    ...
    try {
      ...
      if (response.success) {
        ...
        _appliedCoupon = response.appliedCoupon;
        print("✅ Quantity incremented");
        notifyListeners(); // User action, notify
        _setLoadingState(false);
        return true;
      } else {
        _error = response.message;
      ...
      if (response.success) {
        ...
        if (returnedCart != null &&
          ...
          final reloadUserId =
              _userId.isNotEmpty ? _userId : usrIdd.toString();
          await loadCart(reloadUserId);
          // loadCart will notify if changed
        }
